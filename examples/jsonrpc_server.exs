# A2A JSON-RPC Server Example
#
# Run: elixir examples/jsonrpc_server.exs
# Then run the client in another terminal: elixir examples/jsonrpc_client.exs
...
defmodule MockAgent do
  ...
  Demonstrates sync and streaming responses via JSON-RPC.
  """
  ...
  def handle_stream_message(request, _ctx, emit) do
    ...
    response_chunks = [
      "Processing via JSON-RPC: ",
      "\"#{message_text}\". ",
      "This is chunk 3 of the response. ",
      "Streaming works with JSON-RPC too! ",
      "Final chunk arriving now."
    ]
...
# Combined plug that serves agent card and JSON-RPC endpoints
defmodule ServerPlug do
  ...
  def agent_card do
    A2A.Types.AgentCard.from_map(%{
      "name" => "A2A Example Agent (JSON-RPC)",
      "description" => "An example agent demonstrating the A2A protocol over JSON-RPC transport",
      "version" => "1.0.0",
...
IO.puts("""
Starting A2A JSON-RPC Server on http://localhost:4001

Endpoints:
  GET  /.well-known/agent-card.json  - Agent discovery

JSON-RPC Methods (v0.3.0 naming):
  message/send      - Send message (sync)
  message/stream    - Send message (streaming)
  tasks/get         - Get task
...
{:ok, _} = Plug.Cowboy.http(ServerPlug, [], port: 4001)
