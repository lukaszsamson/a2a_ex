...
#
# Start the server first: elixir examples/jsonrpc_server.exs
# Then run: elixir examples/jsonrpc_client.exs
...
defmodule Demo do
  ...
  def run do
    ...
    IO.puts("A2A JSON-RPC Client Demo")
    IO.puts("=" |> String.duplicate(60))
    IO.puts("")
  ...
  defp demo_discover do
    ...
    # Use JSONRPC transport for discovery
  ...
  defp demo_send_message do
    IO.puts("-" |> String.duplicate(40))
    IO.puts("Demo 2: Send Message (Sync via JSON-RPC)")
    IO.puts("-" |> String.duplicate(40))
    ...
    msg =
      message(
        ...
        role: :user,
        parts: [text_part(text: "Hello from JSON-RPC client!")]
      )
  ...
  defp demo_stream_message do
    IO.puts("-" |> String.duplicate(40))
    IO.puts("Demo 3: Stream Message (via JSON-RPC)")
    IO.puts("-" |> String.duplicate(40))
    ...
    msg =
      message(
        message_id: "msg-#{System.unique_integer([:positive])}",
        role: :user,
        parts: [text_part(text: "Tell me about JSON-RPC streaming!")]
      )
  ...
  defp demo_cancel_task do
    ...
    case A2A.Client.send_message(@base_url, [message: msg], transport: A2A.Transport.JSONRPC) do
      {:ok, %{id: task_id}} ->
        ...
        case A2A.Client.cancel_task(@base_url, task_id, transport: A2A.Transport.JSONRPC) do
          {:ok, task} ->
  ...
  defp demo_error_handling do
    ...
    case A2A.Client.get_task(@base_url, "non-existent-task-id", transport: A2A.Transport.JSONRPC) do
      {:ok, _task} ->
